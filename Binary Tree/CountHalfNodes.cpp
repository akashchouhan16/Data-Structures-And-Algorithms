...
// Iterative
int CountHalfNodesRec(Tree *root)
{
    if (!root)
        return 0;
    int count = 0;
...
int main()
{
    ...
    Inorder(root);
    cout << "\nNumber of Half Nodes  = " << CountHalfNodes(root) << endl
         << "Number of Half Nodes(Recurr) = " << CountHalfNodesRec(root) << endl;
    free(root);
    return 0;
}