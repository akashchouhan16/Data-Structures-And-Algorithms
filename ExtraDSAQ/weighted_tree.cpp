...
{
    int n; cin >> n;
    vector<int> v(n), parent(n + 1, -1), permute(n);
    int root;
    for (int i = 0; i < n; i++)
    {
        cin >> v[i];
    ...
    if (permute[0] != root) {cout << -1 << endl; return;}
    vector<int> curr_dist(n + 1, -1), overall_dist(n + 1, -1);
    ...
    for (int i = 0; i < n; i++)
    {
        if (curr_dist[parent[permute[i]]] == -1)
        {
            cout << -1 << endl;
            return;
        }
        if (permute[i] != root)
        {
            overall_dist[permute[i]] = cnt;
            curr_dist[permute[i]] = overall_dist[permute[i]] - overall_dist[parent[permute[i]]];
        }
        cnt++;
        ans.pb({permute[i], curr_dist[permute[i]]});
    }
    sort(all(ans));
